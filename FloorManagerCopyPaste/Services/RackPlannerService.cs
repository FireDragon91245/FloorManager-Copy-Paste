...
internal static class RackPlannerService
{
    ...
    public static RackTemplate CaptureRackTemplate(RackRuntimeInfo rackInfo, string explicitName = null)
    {
        ...
        var template = new RackTemplate
        {
            ...
            SlotEncodingVersion = RackTemplate.CurrentSlotEncodingVersion,
            TotalSlots = rackInfo.TotalSlots,
    ...
    public static IReadOnlyList<RackTemplate> LoadTemplates()
    {
        try
        {
            ...
            return NormalizeLegacyTemplateSlots(templates ?? []);
        }
        catch (Exception ex)
        {
    ...
    /// <summary>
    /// Templates written before the current slot encoding may contain the stale
    /// lifecycle RackPosition or the incorrectly center-adjusted visual anchor of a
    /// multi-U device. Recalculate those starts from the saved rack-local pose when
    /// the original rack is still available. Device order is deliberately preserved
    /// because cable endpoints address devices by list index.
    /// </summary>
    private static IReadOnlyList<RackTemplate> NormalizeLegacyTemplateSlots(List<RackTemplate> templates)
    {
        if (templates.Count == 0 || templates.All(template =>
                template is null ||
                template.SlotEncodingVersion >= RackTemplate.CurrentSlotEncodingVersion))
            return templates;

        var racks = GetRackInfos();
        var normalized = new List<RackTemplate>(templates.Count);
        foreach (var template in templates)
        {
            ...
            var sourceRack = FindTemplateSourceRack(racks, template.SourceRackLabel);
            if (sourceRack?.Rack is null || template.Devices is null || template.Devices.Any(device => device is null))
            {
                normalized.Add(template);
                continue;
            }

            var devices = new List<RackDeviceTemplate>(template.Devices.Count);
            foreach (var device in template.Devices)
            {
                var startIndex = device.StartIndex;
                if (device.HasLocalPose)
                {
                    var worldPosition = sourceRack.Rack.transform.TransformPoint(device.LocalPos.ToUnity());
                    var visualStart = ResolveVisualBottomPhysicalSlot(
                        sourceRack.Rack, worldPosition, Math.Max(1, device.SizeInU));
                    if (visualStart >= 0)
                        startIndex = visualStart;
                }

                devices.Add(CloneDeviceTemplate(device, startIndex));
            }

            normalized.Add(new RackTemplate
            {
                Name = template.Name,
                SourceRackLabel = template.SourceRackLabel,
                CreatedUtc = template.CreatedUtc,
                SlotEncodingVersion = RackTemplate.CurrentSlotEncodingVersion,
                TotalSlots = sourceRack.TotalSlots,
                Devices = devices,
                Cables = template.Cables ?? []
            });
        }
    ...
    private static RackRuntimeInfo FindTemplateSourceRack(
        IReadOnlyList<RackRuntimeInfo> racks, string sourceRackLabel)
    {
        var exact = racks.FirstOrDefault(rack =>
            string.Equals(rack.Label, sourceRackLabel, StringComparison.Ordinal));
        if (exact is not null)
            return exact;

        // The "Rack N" portion depends on runtime discovery order and can change
        // after a reload. The coordinate suffix is stable for the same placed rack.
        var coordinateStart = sourceRackLabel?.LastIndexOf(" (", StringComparison.Ordinal) ?? -1;
        if (coordinateStart < 0)
            return null;

        var coordinateSuffix = sourceRackLabel.Substring(coordinateStart);
        return racks.FirstOrDefault(rack =>
            rack.Label.EndsWith(coordinateSuffix, StringComparison.Ordinal));
    }
    ...
    /// <summary>
    /// Converts the template's canonical physical bottom slot to the physically
    /// highest occupied RackPosition used as vanilla's insertion anchor. The rack's
    /// internal positions array runs from that upper anchor toward lower physical
    /// slots as IsPositionAvailable/MarkPositionAsUsed increment the array index.
    ///
    /// This is deliberately paste-specific. Capture computes canonical StartIndex
    /// independently in ResolveVisualBottomPhysicalSlot.
    /// </summary>
    private static int ResolvePasteAnchorPhysicalSlot(int bottomPhysicalSlot, int sizeInU)
        => bottomPhysicalSlot + Math.Max(1, sizeInU) - 1;

    private static int ResolvePasteAnchorArrayIndex(Rack rack, int bottomPhysicalSlot, int sizeInU)
        => PhysicalToArrayIndex(rack, ResolvePasteAnchorPhysicalSlot(bottomPhysicalSlot, sizeInU));

    /// <summary>
    /// Resolves the visually occupied bottom U from the device's rack-local vertical
    /// position. In the current game, a mounted device's transform is aligned with
    /// its physically highest occupied RackPosition. RackPosition/currentRackPosition
    /// cannot be trusted for capture because it may retain a different lifecycle
    /// anchor. Interpolate the visual transform against the actual RackPosition
    /// transforms, then subtract the remaining occupied span.
    /// </summary>
    private static int ResolveVisualBottomPhysicalSlot(Rack rack, Vector3 worldPosition, int sizeInU)
    {
        if (!rack || rack.positions is null)
            return -1;

        var ordered = GetPhysicalOrderPositions(rack);
        if (ordered.Count <= 0) return -1;
        if (ordered.Count == 1) return 0;

        var rackTransform = rack.transform;
        var targetY = rackTransform.InverseTransformPoint(worldPosition).y;
        var slotYs = new float[ordered.Count];
        for (var i = 0; i < ordered.Count; i++)
            slotYs[i] = rackTransform.InverseTransformPoint(ordered[i].transform.position).y;

        float centerSlot;
        if (targetY <= slotYs[0])
        {
            var step = slotYs[1] - slotYs[0];
            centerSlot = Mathf.Abs(step) > 0.00001f ? (targetY - slotYs[0]) / step : 0f;
        }
        else if (targetY >= slotYs[^1])
        {
            var last = slotYs.Length - 1;
            var step = slotYs[last] - slotYs[last - 1];
            centerSlot = Mathf.Abs(step) > 0.00001f
                ? last + (targetY - slotYs[last]) / step
                : last;
        }
        else
        {
            centerSlot = 0f;
            for (var i = 0; i < slotYs.Length - 1; i++)
            {
                if (targetY < slotYs[i] || targetY > slotYs[i + 1]) continue;
                var step = slotYs[i + 1] - slotYs[i];
                centerSlot = Mathf.Abs(step) > 0.00001f
                    ? i + (targetY - slotYs[i]) / step
                    : i;
                break;
            }
        }

        var size = Math.Max(1, sizeInU);
        var upperSlot = Mathf.RoundToInt(centerSlot);
        var bottom = upperSlot - (size - 1);
        return Mathf.Clamp(bottom, 0, Math.Max(0, ordered.Count - size));
    }
        ...
        out Server server, out NetworkSwitch sw, out PatchPanel patch)
    {
        ...
        var visualStartIndex =
            ResolveVisualBottomPhysicalSlot(rackPos.rack, usableObject.transform.position, sizeInU);
        var startIndex = visualStartIndex >= 0 ? visualStartIndex : anchorIndex;
        ...
        RackDeviceTemplate template, UsableObject usableObject)
    {
        ...
        try
        {
            ...
            var arrayStart = ResolvePasteAnchorArrayIndex(rack, template.StartIndex, sizeU);
    ...
    private static bool TrySpawnIntoRack(Rack rack, RackDeviceTemplate template, out string message)
    {
        ...
        {
            var sizeU = Math.Max(1, template.SizeInU);
            var anchorPhysicalSlot = ResolvePasteAnchorPhysicalSlot(template.StartIndex, sizeU);
            if (rack.positions is null ||
                template.StartIndex < 0 ||
                template.StartIndex + sizeU > rack.positions.Count ||
                anchorPhysicalSlot >= rack.positions.Count)
            {
                message = $"{template.DisplayName}: invalid target position.";
                return false;
            }

            // StartIndex remains the canonical physical bottom slot. Convert it to
            // vanilla's upper RackPosition anchor only at this paste boundary.
            var arrayStart = ResolvePasteAnchorArrayIndex(rack, template.StartIndex, sizeU);
            if (arrayStart < 0)
            {
                message = $"{template.DisplayName}: invalid target position.";
    ...
    private static RackDeviceTemplate CloneDeviceTemplate(RackDeviceTemplate source, int? startIndex = null)
    {
        return new RackDeviceTemplate
        {
            Kind = source.Kind,
            StartIndex = startIndex ?? source.StartIndex,
